...
- `notebooklm configure` - set chat persona, mode, or response length
...
| Set chat mode | `notebooklm configure --mode detailed` |
| Set custom persona | `notebooklm configure --persona "Act as a research analyst"` |
| Set response length | `notebooklm configure --response-length longer` |
| Configure (specific notebook) | `notebooklm configure --persona "..." -n <notebook_id>` |
| Add URL source | `notebooklm source add "https://..."` |
| Add file | `notebooklm source add ./file.pdf` |
| Add YouTube | `notebooklm source add "https://youtube.com/..."` |
| List sources | `notebooklm source list` |
| Delete source by ID | `notebooklm source delete <source_id> --yes` |
| Delete source by exact title | `notebooklm source delete-by-title "Exact Title" --yes` |
...
| Delete notebook | `notebooklm notebook delete <id> --yes` |
...
**Source filtering:** Use `-s`/`--source` to include specific sources or `-x`/`--exclude-source` to exclude sources from queries and generation. Exclusion is preferred over deletion for quality filtering: it's faster, reversible, and enables multi-topic source juggling within a single notebook.

| Task | Command |
|------|---------|
| Ask using only specific sources | `notebooklm ask "question" -s <id1> -s <id2>` |
| Ask excluding bad sources | `notebooklm ask "question" -x <bad_id1> -x <bad_id2>` |
| Generate excluding sources | `notebooklm generate audio "prompt" -x <bad_id>` |
| Combine include + exclude | `notebooklm ask "question" -s <id1> -s <id2> -x <id3>` |
...
| **Programmatic chat config** | `configure --persona "..."` | Set custom system prompt, mode, and response length without the UI |
...
**Deep research import duplication:** `research wait --import-all` with short timeouts can trigger a retry loop where each retry re-imports all found sources, creating 3-5x duplication. Always use `--timeout 600` or higher for deep research to prevent this. If you observe post-import source counts far exceeding what the research found, duplication has occurred. Delete duplicates before proceeding.
...
Configure a notebook's chat behavior — persona, mode, and response length. This maps to the "Configure Chat" panel in the NotebookLM UI (the settings icon next to Chat/Studio tabs).

**Important:** Chat configuration is **per-notebook**, not global. Each notebook can have its own persona and settings.

```bash
# Set a predefined mode
notebooklm configure --mode learning-guide          # Educational focus
notebooklm configure --mode detailed                 # Verbose, comprehensive
notebooklm configure --mode concise                  # Brief, to-the-point
notebooklm configure --mode default                  # Reset to default behavior

# Set a custom persona (up to 10,000 chars)
notebooklm configure --persona "You are a research analyst. Always cite sources by name."

# Set response length
notebooklm configure --response-length longer        # More detailed responses
notebooklm configure --response-length shorter        # More concise responses
notebooklm configure --response-length default        # Standard length

# Combine options
notebooklm configure --persona "Act as a PhD advisor" --response-length longer

# Target a specific notebook (required in parallel workflows)
notebooklm configure --persona "..." -n <notebook_id>
```

**Modes vs. Persona:**
- `--mode` selects a predefined behavior profile (default, learning-guide, concise, detailed)
- `--persona` sets a custom system prompt — this is the "Custom" tab in the UI
- When `--persona` is set, it overrides `--mode`

**Use cases:**
- Research notebooks: set a persona that enforces citation discipline and gap identification
- Learning notebooks: use `--mode learning-guide` for educational responses
- Quick lookups: use `--mode concise` or `--response-length shorter`
